...
t0 = process_time()
result_df = pd.DataFrame(columns = ['Ticker', 'Close', 'SMA100', 'Stoch RSI', 'ADX', 'Action'])
...
for i in range(0, len(ticker_list)):
	...
	#SMA100
	sma100 = round(c[len(c)-1]/statistics.mean(c[(len(c)-100):(len(c))]), 4)
	...
	for i in range(0, len(c)):
		...
		if i == 14:
			...
			DIu = 100*DMu/tr
			DId = 100*DMd/tr
			adx = 100*abs(DIu - DId)/abs(DIu + DId)
		elif i > 14:
			tr = (tr*13 + max(h[i]-l[i], abs(h[i]-c[i-1]), abs(l[i]-c[i-1])))/14
	...
	stoRSI = 0
	for i in range(len(RSI)-SmoothK, len(RSI)):
		stoRSI += 100*(RSI[i] - min(RSI[(i-13):(i+1)]))/(max(RSI[(i-13):(i+1)]) - min(RSI[(i-13):(i+1)]))
...
result_df['SMA100'] = SMA100
result_df['Stoch RSI'] = StoRSI
result_df['ADX'] = ADX
